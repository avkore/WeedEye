...
if (isset($_POST["query"])) {
    ...
    WHERE (o.status=1) AND (u.name like '%" . $search . "%' OR o.order_id like '%" . $search . "%' OR u.email like '%" . $search . "%')";
} else {
    $start_from = $_POST['myVariable'];
    $limit = 10;
    $query = "SELECT * FROM orders WHERE status=1 LIMIT $start_from, $limit";  
}
...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row1 = mysqli_fetch_array($result)) {
		...
		<td>' . 'Name: ' . $userName . '<br>  Email: ' . $email . '<br> Phone: ' . $phone . '<br> Address: ' . $address . '</td>
		<td>' . $row1["products"] . '</td>
        <td>' . '₾' . $row1["price"] . '</td>
        <td>' . '' . $row1["Shipping"] . '</td>
        <td>' . '' . $row1["Payment"] . '</td>
        ...
        <button class="button small red --jb-modal" onclick="sendVariable(' . $row1['order_id'] . ', "order");" data-target="sample-modal" type="button">
